...
class AccountList extends StatelessWidget {
  ...
  List<AccountItemModel> getOptions(isLoggedIn) => [
        if (!isLoggedIn)
          AccountItemModel(
              icon: 'ic_logout.png', title: 'sign_in'.tr(), onClick: logout, onClickNavigateTo: LoginScreen.routeName),
        if (isLoggedIn)
          AccountItemModel(
              icon: 'ic_order.png', title: 'view_my_orders'.tr(), onClickNavigateTo: OrdersScreen.routeName),
        if (isLoggedIn)
          AccountItemModel(
              icon: 'ic_empty_wishlist.png', title: 'wishlist'.tr(), onClickNavigateTo: WishListScreen.routeName),
        AccountItemModel(
            icon: 'ic_language.png', title: 'change_language'.tr(), onClickNavigateTo: LanguagesScreen.routeName),
        AccountItemModel(
            icon: 'ic_currency.png', title: 'currency'.tr(), onClickNavigateTo: CurrenciesScreen.routeName),
        if (isLoggedIn)
          AccountItemModel(icon: 'ic_logout.png', title: 'logout'.tr(), onClick: logout, showArrow: false),
      ];
